...
class Specs(Dataset):
    def __init__(self, data_dir,dataset,clean_dir, noisy_dir, subset, dummy, supervised,loss_type,shuffle_spec, num_frames,
        ...
        if format == "default":
            if dataset == "tcd" :
                ...
                ##this is possible bcoz each clean speech file has a single corresponding noisy file
                if subset=="train":
                    self.clean_files = sorted(find_files(join(clean_dir, "wsj0_si_tr_s"), ext='wav'))
                    self.noisy_files = sorted(find_files(join(noisy_dir,"train"), ext='wav'))  
                
                elif subset=="valid":
                    self.clean_files = sorted(find_files(join(clean_dir, "wsj0_si_dt_05"), ext='wav'))
                    self.noisy_files = sorted(find_files(join(noisy_dir,"val"), ext='wav'))                      
                
                elif subset=="test":
                    self.clean_files = sorted(find_files(join(clean_dir, "wsj0_si_et_05"), ext='wav'))
                    self.noisy_files = sorted(find_files(join(noisy_dir,"test"), ext='wav'))              
...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument("--dataset", type=str, choices=("tcd", "wsj0"), default="tcd", help="whether to use tcd-timit dataset or wsj0")
        
        parser.add_argument("--clean_dir", type=str, default="none", help="path to the clean speechs dir. This dir should contain train, val, test for clean speechs.Only valid for wsj0 dataset or similarly organised dataset")
        
        parser.add_argument("--noisy_dir", type=str, default="none", help="This dir should contain train, val, test for noisy speechs. Only valid for wsj0 dataset or similarly organised dataset")        
    ...
    def __init__(
        self, base_dir, clean_dir, noisy_dir, dataset="tcd", format='default', batch_size=8,
        n_fft=510, hop_length=128, num_frames=256, window='hann',
        num_workers=4, dummy=False, supervised=False,spec_factor=0.15, spec_abs_exponent=0.5,
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:  
            self.train_set = Specs(data_dir=self.base_dir,dataset= self.dataset,clean_dir=self.clean_dir, noisy_dir=self.noisy_dir, subset='train',
                dummy=self.dummy,supervised=self.supervised, loss_type =self.kwargs.get('loss_type') ,shuffle_spec=True, format=self.format,
                normalize=self.normalize, **specs_kwargs)
            self.valid_set = Specs(data_dir=self.base_dir,dataset= self.dataset,clean_dir=self.clean_dir, noisy_dir=self.noisy_dir, subset='valid',
                dummy=self.dummy,supervised=self.supervised, loss_type =self.kwargs.get('loss_type') ,shuffle_spec=False, format=self.format,
                normalize=self.normalize, **specs_kwargs)
        if stage == 'test' or stage is None:
            self.test_set = Specs(data_dir=self.base_dir,dataset= self.dataset,clean_dir=self.clean_dir, noisy_dir=self.noisy_dir, subset='test',
                dummy=self.dummy,supervised=self.supervised,loss_type =self.kwargs.get('loss_type') ,shuffle_spec=False, format=self.format,
                normalize=self.normalize, **specs_kwargs)
